...
const BootScreen = ({ onBootComplete }) => {
  ...
  useEffect(() => {
    // Progress bar ki speed yahan se set hoti hai
    const interval = setInterval(() => {
      setProgress((prev) => {
        if (prev >= 100) {
          clearInterval(interval);
          setTimeout(onBootComplete, 800); // 100% hone ke baad thoda delay
          return 100;
        }
        return prev + 1; 
      });
    }, 30); // 3 seconds me load hoga approx
  ...
  return (
    <div className="fixed inset-0 bg-black flex flex-col items-center justify-center z-[99999] font-sans">
      ...
      {/* Windows "Spinning Dots" style setup (Optional) */}
