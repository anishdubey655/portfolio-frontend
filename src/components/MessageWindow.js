...
const MessageWindow = ({ onClose, messages, onSend }) => {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center z-[5000] p-0 md:p-4">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={onClose}></div>

      <div className="relative w-full h-full md:max-w-[850px] md:h-[580px] bg-[#1c1c1c] md:rounded-lg shadow-2xl flex flex-col overflow-hidden border border-white/10 text-white">
        
        <div className="h-12 md:h-10 bg-[#202020] flex items-center justify-between pl-3 select-none border-b border-white/5">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="hidden md:flex w-[300px] bg-[#252525] border-r border-white/5 flex-col">
            <div className="p-5">
          ...
          <div className="flex-1 flex flex-col bg-[#1e1e1e]">
            <div className="md:hidden p-3 bg-[#252525] border-b border-white/5 flex items-center gap-3">
               <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center font-bold text-xs">AD</div>
               <span className="text-sm font-semibold">Anish Dubey</span>
            </div>
            ...
            <div className="flex-1 p-4 md:p-6 flex flex-col gap-4 overflow-y-auto">
              {messages.map((msg) => (
                <div 
                  key={msg.id} 
                  className={`${msg.sender === 'User' ? 'self-end' : 'self-start'} max-w-[90%] md:max-w-[85%]`}
                >
                  {msg.sender !== 'User' && (
                    <p className="text-[10px] text-gray-500 ml-2 mb-1 font-bold uppercase">Anish Dubey</p>
                  )}
            ...
            <div className="p-3 md:p-4 bg-[#202020]/50 border-t border-white/5">
              <div className="flex items-center gap-3 bg-[#2d2d2d] rounded-full px-4 md:px-5 py-2 md:py-3 border border-white/10 focus-within:border-blue-500/50 transition-all">
                <input 
                  type="text" 
                  value={message}
                ...
                <button 
                  ...
                  disabled={loading}
                  className={`${loading ? 'bg-gray-500' : 'bg-blue-600 hover:bg-blue-500'} p-2 rounded-full transition-all active:scale-90`}
                >
